...
namespace PixivDownload
{
    ...
    class Pixiv
    {
        ...
        public DownloadProcess downloadImages(String id, bool isSame = false)
        {
            ...
            return new DownloadProcess(httpRequest, url, downloadDirectory);
    ...
    class DownloadProcess
    {
        ...
        public void run()
        {
            ...
            for (; ; picPage++)
            {
                try
                {
                    String tempUrl = url.Replace("p0", "p" + picPage);
                    if (Directory.Exists(downloadDirectory))
                    {
                        request.download(tempUrl, downloadDirectory + "\\" + System.IO.Path.GetFileName(tempUrl), url);
                        Console.Write("\npicture url:" + url);
                        Console.Write("\nname:" + System.IO.Path.GetFileName(url));
                    }
                }
        ...
        public DownloadProcess(HttpRequest request, String url, String downloadDirectory)
        {
